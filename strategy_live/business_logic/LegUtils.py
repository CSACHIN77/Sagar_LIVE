...
async def stoploss_trail(leg, ltp, trade_position):
    """
    This function dynamically adjusts the stop-loss (SL) price for a given trading leg
    based on market movement (LTP). It supports both 'long' and 'short' trade positions
    and utilizes the trailing stop-loss configuration to incrementally modify SL.

    Parameters:
    - leg: Instance of the LegBuilder class representing the trading leg.
    - ltp: Latest traded price of the instrument.
    - trade_position: String indicating the trade direction ('long' or 'short').

    Adjustments:
    - Checks if price movement exceeds configured threshold (trailing_sl["priceMove"]).
    - Calculates new SL based on the movement and applies it via the broker's API.
    - Logs updates and ensures the SL modification is reflected in the leg's attributes.
    """

    if (leg.trailing_sl) and trade_position =='long':
        price_gap = ltp - leg.entry_price
        if price_gap > leg.trailing_sl["priceMove"]:
            orders = leg.interactive.orders

            trail_increment_factor = price_gap//leg.trailing_sl["priceMove"]
            original_sl = leg.trade_entry_price - leg.stop_loss
            updated_sl = original_sl + trail_increment_factor*leg.trailing_sl["sl_adjustment"]
            if updated_sl > leg.sl_price:
                print(f"sl {leg.sl_price} adjusted to {updated_sl} current_price is {ltp} and entry_price {leg.entry_price} in {leg.leg_name}")
                leg.sl_price = updated_sl
                order_uid = f"{leg.leg_name}_sl"
                ...
                params = {
                    ...
                    "modifiedOrderQuantity": leg.lot_size*leg.total_lots,
                    "modifiedDisclosedQuantity": 0,
                    "modifiedLimitPrice": float(updated_sl-leg.trigger_tolerance),
                    "modifiedStopPrice": float(updated_sl),
                    "modifiedTimeInForce": "DAY",
                    "orderUniqueIdentifier": order_uid
                    }
                leg.xts.modify_order(params)
                leg.strategy.logger.log(f'{leg.leg_name} : {leg.instrument.tradingsymbol}, SL updated to {leg.sl_price}')
            else:
                pass
    elif (leg.trailing_sl) and (trade_position=='short') :       
        price_gap = leg.entry_price - ltp 
        if price_gap > leg.trailing_sl["priceMove"]:
            orders = leg.interactive.orders
            print(leg.stop_loss, leg.trade_entry_price)
            trail_increment_factor = price_gap//leg.trailing_sl["priceMove"]
            original_sl =  leg.stop_loss + leg.trade_entry_price
            updated_sl = original_sl - trail_increment_factor*leg.trailing_sl["sl_adjustment"]
            if updated_sl < leg.sl_price:
                print(f"sl {leg.sl_price} adjusted to {updated_sl} current_price is {ltp} and entry_price {leg.entry_price}")
                leg.sl_price = updated_sl
                order_uid = f"{leg.leg_name}_sl"
                for order in orders[::-1]:
                    if order["OrderUniqueIdentifier"] ==order_uid:
                ...
                params = {
                    ...
                    "modifiedProductType": "NRML",
                    "modifiedOrderType": "STOPLIMIT",
                    "modifiedOrderQuantity": int(leg.lot_size*leg.total_lots),
                    "modifiedDisclosedQuantity": 0,
                    "modifiedLimitPrice": float(updated_sl+leg.trigger_tolerance),
                    "modifiedStopPrice": float(updated_sl),
                    "modifiedTimeInForce": "DAY",
