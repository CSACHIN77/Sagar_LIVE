...
#                                               OrderPrice, OrderAverageTradedPrice, ExchangeTransactTime, OrderUniqueIdentifier)
...
#                        ExchangeTransactTime=VALUES(ExchangeTransactTime),
#                        OrderUniqueIdentifier=VALUES(OrderUniqueIdentifier);"""
#             data['ExchangeTransactTime'] = datetime.strptime(data['ExchangeTransactTime'], '%Y-%m-%d %H:%M:%S')

#             data['ExchangeTransactTime'] = data['ExchangeTransactTime'].strftime('%Y-%m-%d %H:%M:%S')
...
#                           data['ExchangeTransactTime'], data['OrderUniqueIdentifier'])
    ...
    try:
        ...
        if connection.is_connected():
            ...
            query = f"""INSERT INTO {table_name} (AppOrderID, TradingSymbol, ExchangeInstrumentID, OrderSide, OrderType,
                                              OrderPrice,Quantity, OrderAverageTradedPrice, ExchangeTransactTime, 
            ...
            data['ExchangeTransactTime'] = datetime.strptime(data['ExchangeTransactTime'], '%Y-%m-%d %H:%M:%S')
            data['ExchangeTransactTime'] = data['ExchangeTransactTime'].strftime('%Y-%m-%d %H:%M:%S')
            # data['ExchangeTransactTime'] = datetime.strptime(data['ExchangeTransactTime'], '%Y-%m-%dT%H:%M:%S.%fZ')
            # data['ExchangeTransactTime'] = data['ExchangeTransactTime'].strftime('%Y-%m-%d %H:%M:%S')
            print(f"update tradebook has data in this format {data}")
            tuple_data = (data['AppOrderID'], data['TradingSymbol'], data['ExchangeInstrumentID'], data['OrderSide'], 
                          data['OrderType'], data['OrderPrice'], data['CumulativeQuantity'], data['OrderAverageTradedPrice'], 
                          data['ExchangeTransactTime'], data['OrderUniqueIdentifier'], price)
...
def report_generator(strategy):
    ...
    entry_df = entry_df.sort_values(by=['TradingSymbol', 'ExchangeTransactTime'])
    exit_df = exit_df.sort_values(by=['TradingSymbol', 'ExchangeTransactTime'])
    ...
    df.rename(columns={
        ...
        'Quantity_x': 'Quantity',
        'ExchangeTransactTime_x': 'entry_time',
        'ExchangeTransactTime_y': 'exitTimeStamp',
