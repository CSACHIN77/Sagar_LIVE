...
class LegBuilder:
    ...
    async def receive_trades(self, data):
        ...
        try:
            dt = datetime.strptime(data['ExchangeTransactTime'], '%Y-%m-%dT%H:%M:%S.%fZ')
            data['ExchangeTransactTimeAPI'] = dt.strftime('%Y-%m-%d %H:%M:%S')
        except Exception as e:
            print(f"Error parsing date: {e}")
        
        # Check for 'sl' suffix and handle accordingly
        if not data['OrderUniqueIdentifier'].endswith('sl'):
            self.trade_data_event.set()
        
        # Handle specific cases based on OrderUniqueIdentifier suffix
        if data['OrderUniqueIdentifier'].endswith('sl'):
            # update_tradebook(data, self.sl_price) #TODO: check if this is needed

            self.trade_position = None
            print('changed trade_position to none')
            print(f'no of reentry {self.no_of_reentry} and reentry count {self.reentry_count}')
        ...
        print(data['ExchangeTransactTime'])
    ...
    async def receive_data(self,data):
        ...
        # print(f"data is coming in receive data {data}")
    ...
    async def reentry_logic(self):
            ...
            if self.strategy.last_entry_time > time_now:
                ...
                if (self.position.lower() =='buy') and (not order_placed):
                    if current_price >= self.entry_price:
                        ...
                        order_placed=True
                        self.strategy.logger.log(f'{self.leg_name} : {self.instrument.tradingsymbol}, Re-entry, {self.position} order placed at {self.entry_price}', self.soc.current_data_time)
        ...
        if order_placed:
            ...
            self.strategy.logger.log(f'{self.leg_name} : {self.instrument.tradingsymbol}, Re-entry SL {orderSide} order placed at {self.sl_price}', self.soc.current_data_time)
            self.trade_data_event.clear()
    
    async def stoploss_trail(self, ltp, trade_position):

        print(f"inside stoploss_trail function, {ltp} {self.entry_price} and sl is {self.sl_price}")
        if (self.trailing_sl) and trade_position =='long':
            ...
            if price_gap > self.trailing_sl["priceMove"]:
                ...
                if updated_sl > self.sl_price:
                    ...
                    self.strategy.logger.log(f'{self.leg_name} : {self.instrument.tradingsymbol}, SL updated to {self.sl_price}', self.soc.current_data_time)
                else:
                    pass
    ...
    def selection_criteria(self):
        ...
        if  choice == 'strike':
            if choice_value=='ATM':
                # print(self.expiry_df, type(self.strike), self.strike)
                self.option_symbol = self.expiry_df[(self.expiry_df['strike'].astype(int)== self.strike)]
                print(f"option symbol is {self.option_symbol}")
            ...
            for chunk in chunks:
                ...
                response = self.xts.get_quotes(premium_instruments_chunk, self.soc)
                ltp_data = response['result']['listQuotes']
                # print(f"ltp data {ltp_data}")
                for item in ltp_data:
                    # print(f"item is {item}")
                    item = item.replace('"null"', '0')  #added for sandbox 
                    try:
            ...
            results = self.xts.get_quotes(options_list, self.soc)
            ...
            if choice.lower() == 'atm_straddle_premium':
                ...
                self.instrument_id = int(selected_option.instrument_token)
                self.lot_size = selected_option.lot_size
                print("selected symbol{selected_option.tradingsymbol}")
        ...
        self.strategy.logger.log(f'{self.leg_name} : {self.instrument.tradingsymbol}, entry_price before placing order is {self.entry_price}', self.soc.current_data_time)
        if self.range_breakout:
            ...
            self.strategy.logger.log(f'{self.leg_name} : {self.instrument.tradingsymbol}, order placed for range breakout with entry price {limit_price}', self.soc.current_data_time)
            print(order)
            ...
            order = self.xts.place_market_order({"exchangeInstrumentID": self.instrument_id, "orderSide": self.position,
                                      "orderQuantity":int(self.lot_size) *self.total_lots,"limitPrice":0,
                                        "stopPrice": 1, "orderUniqueIdentifier": self.leg_name})
            self.strategy.logger.log(f'{self.leg_name} : {self.instrument.tradingsymbol} market order placed', self.soc.current_data_time)
    ...
    async def leg_place_order(self):        
        ...
        if latest_trade['OrderStatus']=='Filled':
            ...
            # print(f'instrument traded at avg price of {self.trade_entry_price}, side {trade_side},\n with quantity {traded_quantity} and executed at  {entry_timestmap} and slippage is {entry_slippage}')
            # print(trade)
            self.strategy.logger.log(f'{self.leg_name} : {self.instrument.tradingsymbol}, order filled {self.entry_price}', self.soc.current_data_time)
        # print(f'placing SL order now for {self.leg_name} SL points {self.stop_loss}')
        if self.stop_loss[0].lower()=='points':
            self.stop_loss = self.stop_loss[1]
        elif self.stop_loss[0].lower() == 'percent':
        ...
        self.strategy.logger.log(f'{self.leg_name} : {self.instrument.tradingsymbol}, SL order placed with limit price {self.sl_price}', self.soc.current_data_time)
        # self.pegasus.log(order)
    def order_execution():
        pass
